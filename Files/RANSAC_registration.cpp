...
Mat getOptimumTform(Mat movpoints, Mat srcpoints)
{
    int nsamples = 60;
    int siz;
    int N = 30;
...
//     siz = ( (movpoints.rows() > movpoints.cols)? movpoints.rows:movpoints.cols );
    siz = movpoints.rows;
	cout<<"\n ROWS::::"<<movpoints.rows <<" cols::::" << movpoints.cols <<"\n";
    int j =0;
   // for(int j = 0; j< N; j++)
    while( (r < 0.7) || ( j <= N) )
    {
...
//	cout<<"\n mvgpts size::"<<mvgpts.size()<<"\n";
	...
	{
	warp_m = getOptimumTform(mvgpts,srcpts);
	}
...
Mat register_BG(Mat refframe, Mat movingframe)			// refframe = previous / static frame === movingframe = next / moving frame
{	
	Mat warp_matrix;
	Mat refgray,movgray;
	cvtColor(refframe, refgray,CV_BGR2GRAY);
